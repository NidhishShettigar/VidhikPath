...
try:
    ...
    logger.error(f"Optional summarizer dependency missing: {e}")
...
def clean_image(image_path):
    ...
    if cv2 is None:
        raise RuntimeError("OpenCV is not available in this deployment.")
...
def extract_text_from_pdf(pdf_path):
    ...
    if pdfplumber is None:
        raise RuntimeError("pdfplumber is not available in this deployment.")
    text = ""
...
def ocr_pdf(pdf_path, language='eng'):
    ...
    if convert_from_path is None:
        raise RuntimeError("pdf2image is not available in this deployment.")
...
def summarize_api(request):
    ...
    if DEPENDENCY_ERROR:
        return JsonResponse(
            {"error": f"Summarizer dependencies unavailable: {DEPENDENCY_ERROR}"},
            status=503,
        )
...
def health_check(request):
    ...
    dependencies = {
        ...
        "ocr_dependencies": DEPENDENCY_ERROR is None,
    }
